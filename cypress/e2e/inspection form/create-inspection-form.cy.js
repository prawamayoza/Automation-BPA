...
describe('create', () => {
  it('add in the data inspection without fill out the form', () => {
      ...
      cy.get('#formId-feedback', { timeout: 10000 })
          .should('be.visible')
          .and('contain', 'formId is a required field');
  });

  it('add inspection data fill in inspection', () => {
      cy.contains('button', "Add New", { timeout: 10000 }).should('be.visible').click();
      cy.get('#react-select-2-input', { timeout: 10000 }).click({ force: true });
      cy.get('#react-select-2-listbox').contains('Checklist For Checking Electricity of Home').click();

      // Pilih Team Assignee dengan retry
      cy.contains('label.chakra-form__label', 'Team Assignee', { timeout: 10000 })
          .parent()
          .find('button[aria-label="add-team"]')
          .click();

      cy.contains('span.chakra-checkbox__label', 'TEAM TWO', { timeout: 10000 })
          .parent('label.chakra-checkbox')
          .find('input[type="checkbox"]')
          .check({ force: true });
      ...
      // Klik tombol Assignee
      cy.get('#assignee-label', { timeout: 10000 })
          .closest('.chakra-form-control')
          .find('button[aria-label="add-team"]')
          .click();

      // Uncheck Assignee otomatis
      cy.contains('span.chakra-checkbox__label', 'Rezkya Wolin', { timeout: 10000 })
          .parent('label.chakra-checkbox')
          .find('input[type="checkbox"]')
          .uncheck({ force: true });

      // Check Assignee Baru
      cy.contains('span.chakra-checkbox__label', 'prawamaa@gmail.com', { timeout: 10000 })
          .parent('label.chakra-checkbox')
          .find('input[type="checkbox"]')
          .check({ force: true });

      cy.get('#react-select-4-input', { timeout: 10000 }).click({ force: true });
      cy.get('#react-select-4-listbox').contains('Girudo Digital Team').click();

      // Cek apakah field terisi otomatis
      cy.get('#customerName', { timeout: 10000 }).invoke('val').should('not.be.empty');
      cy.get('#customerLocation', { timeout: 10000 }).invoke('val').should('not.be.empty');

      cy.get('#react-select-5-input', { timeout: 10000 }).click({ force: true });
      cy.get('#react-select-5-listbox').contains('Head Quarter Building').click();

      cy.contains('button', "Save", { timeout: 10000 }).click();
      cy.contains('button', "Confirm", { timeout: 10000 }).click();

      // Verifikasi tombol muncul sebelum klik
      cy.contains('button', "Do Checklist Now!", { timeout: 10000 })
          .should('be.visible')
          .click();
  });
});
